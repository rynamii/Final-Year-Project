...
class BMCLoss:
    def __init__(
        ...
        self.joint_angle_limit = np.load(
            os.path.join(self.lp, "CONVEX_HULLS.npy"), allow_pickle=True
        )
    ...
    def compute_loss(self, joints):
        ...
        BMC_losses = {
            "bmc_bl": torch.Tensor([0]).cuda(),
            "bmc_rb": torch.Tensor([0]).cuda(),
            "bmc_a": torch.Tensor([0]).cuda(),
        }
        ...
        ALL_bones = [
            (joints[:, i, :] - joints[:, cfg.SNAP_PARENT[i], :]) for i in range(21)
        ]
        ...
        if self.lambda_a:
            ...
            DIP_theta_flexion = torch.where(
                tmp < 0, -DIP_theta_flexion, DIP_theta_flexion
            )
            # y-component of the bone vector
            tmp = inner_product(DIP_bones, DIP_Y_axis)
            DIP_theta_abduction = torch.where(
                tmp < 0, -DIP_theta_abduction, DIP_theta_abduction
            )
