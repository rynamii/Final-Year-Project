...
from opendr.common import (
    draw_visibility_image,
    draw_barycentric_image,
...
class BaseRenderer(Ch):
    ...
    @depends_on(
        "f"
    )  # not v: specifically, it depends only on the number of vertices, not on the values in v
    ...
    def boundaryid_image(self):
        self._call_on_changed()
        return draw_boundaryid_image(
            self.glb, self.v.r, self.f, self.vpe, self.fpe, self.camera
        )

    @depends_on("f", "frustum", "camera", "overdraw")
    def visibility_image(self):
        self._call_on_changed()
        return draw_visibility_image(
            self.glb,
            self.v.r,
            self.f,
            self.boundarybool_image if self.overdraw else None,
    ...
    def boundarybool_image(self):
        ...
        return np.asarray(boundaryid_image != 4294967295, np.uint32).reshape(
            boundaryid_image.shape
        )
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if wrt is self.camera:
            ...
            if self.overdraw:
                ...
                result1 = common.dImage_wrt_2dVerts(
                    depth,
                    visible,
                    visibility,
                    barycentric,
    ...
    def depth_image(self):
        ...
        if hasattr(self, "background_image"):
            if False:  # has problems at boundaries, not sure why yet
                ...
                tmp = np.concatenate(
                    (np.atleast_3d(result), np.atleast_3d(self.background_image)),
                    axis=2,
                )
...
class BoundaryRenderer(BaseRenderer):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        return common.dImage_wrt_2dVerts(
            self.color_image,
            visible,
            visibility,
            barycentric,
...
class ColoredRenderer(BaseRenderer):
    ...
    def compute_r(self):
        ...
        return (
            self.color_image
        )  # .reshape((self.frustum['height'], self.frustum['width'], -1)).squeeze()
    ...
    def on_changed(self, which):
        ...
        if not hasattr(self, "bgcolor"):
            self.bgcolor = Ch(np.array([0.5] * self.num_channels))
            which.add("bgcolor")
    ...
    def draw_color_image(self, gl):
        ...
        result = np.asarray(
            deepcopy(gl.getImage()[:, :, : self.num_channels].squeeze()), np.float64
        )
    ...
    def color_image(self):
        ...
        return np.asarray(
            (overdraw * boundarybool_image + no_overdraw * (1 - boundarybool_image)),
            order="C",
        )
...
class TexturedRenderer(ColoredRenderer):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if wrt is self.vc:
            ...
            clr_im = self.draw_color_image(
                with_vertex_colors=True, with_texture_on=False
            )
    ...
    def on_changed(self, which):
        ...
        if "texture_image" in which or "frustum" in which:
            ...
            texture_data = np.array(
                self.texture_image * 255.0, dtype="uint8", order="C"
            )
    ...
    def mesh_tex_coords(self):
        ...
        data = np.asarray(
            self.vt[ftidxs].astype(np.float32)[:, 0:2], np.float32, order="C"
        )
    ...
    def boundarycolor_image(self):
        ...
        colors = self.vc.r.reshape((-1, 3))[self.vpe.ravel()]
        gl.Clear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
        self.texture_mapping_on(
            gl, with_vertex_colors=False if colors is None else True
        )
    ...
    def texcoord_image_quantized(self):
        ...
        texcoord_image = (
            texcoord_image[:, :, 0]
            + texcoord_image[:, :, 1] * self.texture_image.shape[1]
        )
        return texcoord_image

    @depends_on("vt", "ft", "f", "frustum", "camera")
    def texcoord_image(self):
        return draw_texcoord_image(
            self.glf,
            self.v.r,
            self.f,
            self.vt,
...
def draw_boundary_images(glf, glb, v, f, vpe, fpe, camera):
    ...
    rays_to_faces = (
        rays_to_verts[f[:, 0]] + rays_to_verts[f[:, 1]] + rays_to_verts[f[:, 2]]
    )
...
def draw_boundaryid_image(gl, v, f, vpe, fpe, camera):
    ...
    if True:
        ...
        silhouette_edges, faces_facing_camera = compute_vpe_boundary_idxs(
            v, f, camera, fpe
        )
        lines_e = vpe[silhouette_edges]
        lines_v = v
        ...
        visibility = draw_edge_visibility(
            gl, lines_v, lines_e, f, hidden_wireframe=True
        )
        shape = visibility.shape
        visibility = visibility.ravel()
        visible = np.nonzero(visibility.ravel() != 4294967295)[0]
...
def draw_colored_verts(gl, v, f, vc):
    ...
    if vc.shape[1] != 3:
        vc = np.vstack(
            (vc[:, 0], vc[:, 1 % vc.shape[1]], vc[:, 2 % vc.shape[1]])
        ).T.copy()
    assert vc.shape[1] == 3
